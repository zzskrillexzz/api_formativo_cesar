...
def listarFacturas():
    ...
    sql = "SELECT fac_id, fac_fecha_emision, fac_email_enviado, fac_forma_pago, fac_total, fac_usu_id_fk, fac_estado FROM t_factura"
    c.execute(sql)
    reg = c.fetchall()
    lista = []
    for f_row in reg:
        fac = facturas(
            ...
            forma_pago=f_row[3], total=f_row[4], usuario_id=f_row[5], fac_estado=f_row[6]
        ).todic()
        lista.append(fac)
    return lista
...
def registrarFacturas(data):
    try:
        ...
        sql = """
            INSERT INTO t_factura (fac_id, fac_fecha_emision, fac_email_enviado, fac_forma_pago, fac_total, fac_usu_id_fk, fac_estado)
...
def editarFacturas(fac_id, data):
    try:
        ...
        sql = """
            ...
            fac_total=%s, fac_usu_id_fk=%s, fac_estado=%s WHERE fac_id=%s
        """
        cursor.execute(sql, (
...
def buscarFacturas(fac_id):
    try:
        cursor = current_app.mysql.connection.cursor()
        sql = "SELECT fac_id, fac_fecha_emision, fac_email_enviado, fac_forma_pago, fac_total, fac_usu_id_fk, fac_estado FROM t_factura WHERE fac_id = %s"
        cursor.execute(sql, (fac_id,))
        row = cursor.fetchone()
        cursor.close()
        if row:
            return facturas(id=row[0], fecha_emision=row[1], email_enviado=row[2],
                          forma_pago=row[3], total=row[4], usuario_id=row[5], fac_estado=row[6]).todic()
        return None
    except Exception as e:
        raise e
