...
def _descontarInventario(c, det_pro_id_fk, det_lot_id_fk, det_cantidad, det_ped_id_fk, det_precio_unitario, det_subtotal):
    ...
    """
    # ── 1. Validar y descontar producto ──
    ...
    if stock_anterior < det_cantidad:
        raise ValueError(f"Stock insuficiente para {det_pro_id_fk}: disponible {stock_anterior}, solicitado {det_cantidad}")
...
def registrarDetallesPedidos(det_id, det_ped_id_fk, det_pro_id_fk, det_lot_id_fk, det_cantidad, det_precio_unitario, det_subtotal):
    ...
    # Descontar inventario (producto, lote, movimiento, monitoria)
    # _descontarInventario puede auto-asignar un lote si no se especificó; actualizamos el detalle con el lote usado
